...
type ToolRouterBuild = (
    ToolRouter<QuickDepServer>,
    HashSet<String>,
    Option<Vec<String>>,
);
...
impl QuickDepServer {
    ...
    fn build_tool_router(allowed_tools: Option<&[String]>) -> anyhow::Result<ToolRouterBuild> {
...
impl QuickDepServer {
    ...
    pub async fn get_file_interfaces(
        ...
        if self.remote_client().is_some() {
            return Ok(Json(
                self.proxy_tool("get_file_interfaces", &request).await?,
            ));
        }
    ...
    pub async fn get_verification_context(
        ...
        if self.remote_client().is_some() {
            return Ok(Json(
                self.proxy_tool("get_verification_context", &request)
                    .await?,
            ));
        }
    ...
    pub async fn analyze_workflow_context(
        ...
        if self.remote_client().is_some() {
            return Ok(Json(
                self.proxy_tool("analyze_workflow_context", &request)
                    .await?,
            ));
        }
        Ok(Json(
    ...
    pub async fn analyze_behavior_context(
        ...
        if self.remote_client().is_some() {
            return Ok(Json(
                self.proxy_tool("analyze_behavior_context", &request)
                    .await?,
            ));
        }
        Ok(Json(
