...
它完全本地运行，把图数据存进 SQLite，通过单实例 daemon 维持共享运行态，并通过 MCP、HTTP、WebSocket 和本地 Web UI 暴露出来。
...
它直接连接 daemon 托管的本地 QuickDep HTTP 服务，和 MCP 客户端、脚本、API 用的是同一套数据。
...
截至 `2026-04-27`，当前实测有效的安装路径是“源码安装 + `install-mcp` 接入客户端”。GitHub Release 走 tag 自动发布。

| 方式 | 当前状态 | 我们的验证结果 |
| --- | --- | --- |
| `cargo install --path .` | 已可用 | 已实测安装成功，`quickdep --version` 返回 `0.1.2` |
...
| GitHub Release | 打 tag 自动发布 | 推送 `v0.1.2` 到 `embedclaw/QuickDep` 后会触发 release workflow |
...
QuickDep 默认子命令就是 `serve`，所以直接运行即可启动本地 `stdio MCP` 代理；真正的项目状态由后台 daemon 统一持有：

```bash
# 在当前目录启动本地 stdio MCP
quickdep

# 同时暴露 MCP stdio 和 daemon 托管的 HTTP
quickdep --http 8080
